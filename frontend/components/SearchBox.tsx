...
const SearchBox = () => {
  ...
  useEffect(() => {
    ...
    const roomType = searchParams.get("roomType") || "single";
  ...
  const handleCityInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length > 0) {
      const filtered = sampleCities.filter(city =>
        ...
        city.countryName.toLowerCase().includes(value.toLowerCase())
      ).slice(0, 5);
  ...
  const onSubmit = (data: any) => {
    router.push(
      `/stays?city=${data.city}&checkIn=${data.checkIn}&checkOut=${data.checkOut}&roomType=${data.roomType}`
    );
  };

  return (
    <form
      className="bg-white rounded-lg p-4 border border-gray-300 relative"
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div>
          <InputField label="Check-in" type="date" {...register("checkIn")} />
          <p className="error">{errors.checkIn?.message}</p>
        </div>
        <div>
          <InputField label="Check-out" type="date" {...register("checkOut")} />
